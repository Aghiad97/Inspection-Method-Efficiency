...
SELECT 
    STR_TO_DATE(TRIM(defect_date) , '%m/%d/%Y') AS date, -- Formats the date to 'YYYY-MM' for monthly aggregation
    COUNT(defect_id) AS total_defects,               -- Sums the defect IDs found for each month
    inspection_method											
FROM 
    Defects.products_defects_data
GROUP BY 
    1, 3                               -- Group by month (YYYY-MM)
ORDER BY 
    1;                                      -- Order by month for proper time series
